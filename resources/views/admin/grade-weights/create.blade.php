...
@section('title', 'Tambah Bobot Nilai - ' . ($configs['school_name'] ?? 'SesekaliCBT'))
...
<div class="max-w-2xl mx-auto space-y-8 animate-fadeIn pb-12">
    ...
    <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase">Tambah Bobot Nilai</h2>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Konfigurasi persentase bobot untuk satu mapel satu periode</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.grade-weights.store') }}" method="POST" class="space-y-8"
          x-data="{
              harian: {{ old('weight_harian', 40) }},
              uts: {{ old('weight_uts', 30) }},
              uas: {{ old('weight_uas', 30) }},
            ...
            <div class="p-8 space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-[10px] font-black uppercase tracking-widest"
                               :class="isValid ? 'text-emerald-700' : 'text-rose-700'">Total Bobot</p>
                            <p class="text-3xl font-black mt-1"
                               :class="isValid ? 'text-emerald-600' : 'text-rose-600'"
                               x-text="total.toFixed(0) + '%'"></p>
                        </div>
                        <div class="w-12 h-12 rounded-2xl flex items-center justify-center"
                             :class="isValid ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="weight_harian" name="weight_harian"
                                x-model="harian"
                                value="{{ old('weight_harian', 40) }}"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="weight_uts" name="weight_uts"
                                x-model="uts"
                                value="{{ old('weight_uts', 30) }}"
                                min="0" max="100" step="0.5"
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all"
                                required>
                            <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-300 uppercase tracking-widest">%</span>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="weight_uas" name="weight_uas"
                                x-model="uas"
                                value="{{ old('weight_uas', 30) }}"
                                min="0" max="100" step="0.5"
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all"
                                required>
                            <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-300 uppercase tracking-widest">%</span>
        ...
        <div class="flex flex-col-reverse md:flex-row gap-4 pt-4">
            ...
            <button type="submit" class="flex-[2] h-14 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 flex items-center justify-center gap-3 group">
                <i class="fas fa-check-circle text-[10px] group-hover:scale-110 transition-transform"></i> Simpan Konfigurasi
            </button>
        </div>
    </form>
</div>
