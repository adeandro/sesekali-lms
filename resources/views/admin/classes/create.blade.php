...
@section('title', 'Tambah Kelas - ' . ($configs['school_name'] ?? 'SesekaliCBT'))
...
<div class="max-w-2xl mx-auto space-y-8 animate-fadeIn pb-12">
    ...
    <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase">Tambah Kelas</h2>
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Buat kelas baru dan tentukan wali kelasnya</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.classes.store') }}" method="POST" class="space-y-8">
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <select id="grade" name="grade"
                                ...
                                <option value="10" {{ old('grade') == '10' ? 'selected' : '' }}>Kelas 10 (X)</option>
                                <option value="11" {{ old('grade') == '11' ? 'selected' : '' }}>Kelas 11 (XI)</option>
                                <option value="12" {{ old('grade') == '12' ? 'selected' : '' }}>Kelas 12 (XII)</option>
                            </select>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="capacity" name="capacity" value="{{ old('capacity', 36) }}"
                                min="1" max="100"
                ...
                <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-indigo-400 hover:bg-white transition-all duration-200 cursor-pointer overflow-hidden shadow-sm hover:shadow-lg">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="hidden peer">
                    ...
                    <div class="relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out bg-gray-300 peer-checked:bg-indigo-600">
        ...
        <div class="flex flex-col-reverse md:flex-row gap-4 pt-4">
            ...
            <button type="submit" class="flex-[2] h-14 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 flex items-center justify-center gap-3 group">
                <i class="fas fa-check-circle text-[10px] group-hover:scale-110 transition-transform"></i> Buat Kelas
            </button>
        </div>
    </form>
</div>
