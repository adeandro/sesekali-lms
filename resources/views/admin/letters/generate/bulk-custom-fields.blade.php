...
@section('title', 'Lengkapi Data Masal - ' . ($configs['school_name'] ?? 'SesekaliCBT'))

@section('page-title', 'Lengkapi Data Masal')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            <p class="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">📍 Generator — Bulk Custom Fields</p>
            <h2 class="text-2xl font-black tracking-tight" style="color: var(--brand-text);">Lengkapi <span style="color: var(--brand-primary);">Data Masal</span></h2>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm p-8" style="border-color: var(--brand-glow);">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 rounded-3xl bg-amber-50 flex items-center justify-center text-amber-600">
                    <i class="fas fa-layer-group text-2xl"></i>
                </div>
                <div>
                    <h3 class="text-lg font-black" style="color: var(--brand-text);">{{ $template->name }}</h3>
                    <p class="text-sm font-bold text-gray-400 uppercase tracking-tighter">{{ $template->code }} • {{ count($recipientIds) }} Penerima terpilih</p>
                </div>
            </div>
            <div class="flex items-center gap-4 px-6 py-4 bg-amber-50 rounded-2xl border border-dashed border-amber-200">
                <div class="w-10 h-10 rounded-xl bg-white border border-gray-100 flex items-center justify-center text-amber-600">
                    <i class="fas fa-info-circle"></i>
                </div>
                <div class="max-w-[200px]">
                    <p class="text-[10px] font-black uppercase tracking-widest text-amber-900">Informasi</p>
                    <p class="text-[9px] font-bold text-amber-800 leading-tight">Data yang Anda masukkan di sini akan diterapkan pada <span class="font-black">seluruh</span> surat dalam batch ini.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden" style="border-color: var(--brand-glow);">
        ...
        <form action="{{ route('admin.letters.bulk.progress-page', $template) }}" method="POST" class="p-8">
            @csrf
            {{-- Pass back all selected recipient IDs --}}
            ...
            <div class="mt-12 flex items-center justify-end gap-4">
                <button type="submit" 
                        class="flex items-center gap-3 px-8 py-4 bg-amber-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-amber-700 transition shadow-lg shadow-amber-100">
                    Lanjut Generate <i class="fas fa-arrow-right"></i>
                </button>
            </div>
        </form>
    </div>
