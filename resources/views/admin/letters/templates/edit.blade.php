...
@section('title', 'Edit Template - ' . ($configs['school_name'] ?? 'SesekaliCBT'))
...
<div class="max-w-5xl mx-auto space-y-8 animate-fadeIn pb-20">
    ...
    <div class="flex flex-col gap-4">
        <nav class="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
            ...
            <span class="text-indigo-600">Edit Template</span>
        </nav>
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase">Edit Template</h2>
                    <p class="text-[11px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">Perbarui format "{{ $template->name }}"</p>
                </div>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.letters.templates.update', $template) }}" method="POST" id="templateForm" class="space-y-8 relative">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-[3rem] shadow-sm border border-gray-100 p-8 md:p-12 relative overflow-hidden group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $template->code) }}" class="block w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold transition-all placeholder:text-gray-300 @error('code') ring-2 ring-rose-500 @enderror" placeholder="Contoh: SK-SISWA" required>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative">
                        <select id="letter_type_id" name="letter_type_id" class="block w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold appearance-none cursor-pointer @error('letter_type_id') ring-2 ring-rose-500 @enderror">
                            ...
                            <option value="{{ $type->id }}" {{ old('letter_type_id', $template->letter_type_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->name }} ({{ $type->code }})
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative">
                        <select id="category" name="category" class="block w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold appearance-none cursor-pointer @error('category') ring-2 ring-rose-500 @enderror" required>
                            <option value="siswa" {{ old('category', $template->category) == 'siswa' ? 'selected' : '' }}>Administrasi Siswa</option>
                            <option value="guru" {{ old('category', $template->category) == 'guru' ? 'selected' : '' }}>Administrasi Guru</option>
                            <option value="umum" {{ old('category', $template->category) == 'umum' ? 'selected' : '' }}>Umum / Lainnya</option>
                        </select>
                        <i class="fas fa-chevron-down absolute right-6 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none text-[10px]"></i>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $template->sort_order) }}" class="block w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold transition-all @error('sort_order') ring-2 ring-rose-500 @enderror">
        ...
        <div class="bg-white rounded-[3rem] shadow-sm border border-gray-100 p-8 md:p-12 relative overflow-hidden group">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 relative">
                ...
                {{-- Jika template SPPD --}}
                @if($template->code === 'SPPD')
                <div class="bg-blue-50 border border-blue-100
                            rounded-xl p-4 mt-3 text-xs">
                    <p class="font-black text-blue-700 mb-2">
                    Placeholder khusus SPPD (diisi guru saat cetak):
                    </p>
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-1 font-mono
                                text-blue-600">
                    <span>[tujuan]</span>
                    <span>[keperluan]</span>
                    ...
                    <span>[kendaraan]</span>
                    <span>[jabatan_guru]</span>
                    </div>
                </div>
                @endif

                {{-- Jika template SKS-A --}}
                @if($template->code === 'SKS-A')
                <div class="bg-emerald-50 border border-emerald-100
                            rounded-xl p-4 mt-3 text-xs">
                    <p class="font-black text-emerald-700 mb-2">
                    Placeholder self-service (diisi siswa saat cetak):
                    </p>
