...
@section('title', 'Edit Question - SesekaliCBT')

@section('content')
    <div>
        <div class="mb-8">
            <a href="{{ route('admin.questions.index') }}" class="text-blue-600 hover:text-blue-800">← Back to Questions</a>
            <h2 class="text-3xl font-bold text-gray-900 mt-2">Edit Question</h2>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-8 max-w-4xl">
            <form action="{{ route('admin.questions.update', $question) }}" method="POST" id="questionForm" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select id="jenjang" name="jenjang" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('jenjang') border-red-500 @enderror" required>
                            ...
                            <option value="10" {{ old('jenjang', $question->jenjang) == '10' ? 'selected' : '' }}>10</option>
                            <option value="11" {{ old('jenjang', $question->jenjang) == '11' ? 'selected' : '' }}>11</option>
                            <option value="12" {{ old('jenjang', $question->jenjang) == '12' ? 'selected' : '' }}>12</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="topic" name="topic" value="{{ old('topic', $question->topic) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('topic') border-red-500 @enderror" required>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="question_text" name="question_text" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('question_text') border-red-500 @enderror" required>{{ old('question_text', $question->question_text) }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    @if($question->question_image)
                        <div class="mb-3">
                            ...
                            <img src="{{ asset($question->question_image) }}" alt="Question Image" class="max-w-xs h-auto rounded-lg border border-gray-300 mb-2">
                        </div>
                    @endif
                ...
                <div id="multipleChoiceSection" class="mb-6">
                    ...
                    @foreach(['A' => 'option_a', 'B' => 'option_b', 'C' => 'option_c', 'D' => 'option_d', 'E' => 'option_e'] as $label => $name)
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            ...
                            <input type="text" id="{{ $name }}" name="{{ $name }}" value="{{ old($name, $question->$name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mb-3" placeholder="Text for option {{ $label }}">
                            
                            <label for="{{ $name }}_image" class="block text-sm font-medium text-gray-700 mb-2">
                                Image for Option {{ $label }} (optional)
                            </label>
                            @php $fieldNameImage = $name . '_image'; @endphp
                            @if($question->$fieldNameImage)
                                <div class="mb-2">
                                    <p class="text-xs text-gray-600 mb-1">Current Image:</p>
                                    <img src="{{ asset($question->$fieldNameImage) }}" alt="Option {{ $label }} Image" class="max-w-xs h-auto rounded-lg border border-gray-300 mb-2">
                                </div>
                            @endif
                ...
                <div class="mb-6">
                    ...
                    <textarea id="explanation" name="explanation" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('explanation', $question->explanation) }}</textarea>
                </div>
