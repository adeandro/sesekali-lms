...
@section('title', 'Edit Achievement — ' . $achievement->display_title)
@section('page-title', 'Edit Achievement')

@section('content')
<div class="max-w-5xl mx-auto space-y-8 animate-fadeIn pb-12" x-data="achievementEditForm()">
    ...
    <div class="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-gray-400">
        ...
        <span style="color: var(--brand-primary);">{{ $achievement->display_title }}</span>
    </div>
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-3xl font-black tracking-tight" style="color: var(--brand-text);">Edit Pencapaian</h2>
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ $achievement->slug }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 space-y-6">
            <form action="{{ route('admin.gamification.achievements.update', $achievement) }}" method="POST" enctype="multipart/form-data" id="achievementForm">
                @csrf
                    ...
                    <div class="p-8 border-b border-gray-50 flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Informasi Tampilan</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-amber-500/5 transition-all duration-500 mt-6">
                    ...
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        {{-- Color Picker --}}
                        <div class="space-y-4">
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Pilih Warna Dasar</label>
                ...
                <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden mt-6">
                    <div class="p-8 flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <input type="checkbox" name="is_active" id="is_active" value="1" x-model="isActive" class="w-5 h-5 rounded text-purple-600 focus:ring-purple-500 border-gray-300">
                            <div>
                        ...
                        <div class="flex items-center gap-4">
                            <a href="{{ route('admin.gamification.achievements') }}"
                               class="px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-500 hover:bg-gray-100 transition-colors">
            ...
            <div class="sticky top-8 space-y-4">
                ...
                <div class="group relative bg-white rounded-[2rem] border border-gray-100 shadow-xl overflow-hidden will-change-transform" x-data="{ pulse: false }" x-effect="pulse = true; setTimeout(() => pulse = false, 500)">
                    ...
                    <div class="absolute top-3 right-3 z-10">
                        <span class="px-2 py-0.5 bg-emerald-100 text-emerald-600 text-[8px] font-black uppercase tracking-widest rounded-md" x-show="isActive">Aktif</span>
                        <span class="px-2 py-0.5 bg-gray-100 text-gray-500 text-[8px] font-black uppercase tracking-widest rounded-md" x-show="!isActive">Non-Aktif</span>
                    </div>
                ...
                <div class="p-4 bg-purple-50/50 rounded-2xl border border-purple-100 text-center">
                    ...
                    <p class="text-[9px] text-purple-500 mt-1">Sistem kami otomatis mendeteksi kriteria baru ini ke seluruh pengguna.</p>
                </div>
            </div>
        </div>
...
function achievementEditForm() {
    return {
        title: `{!! addslashes(old('title', $achievement->title ?? $achievement->name)) !!}`,
        description: `{!! addslashes(old('description', $achievement->description)) !!}`,
        loreText: `{!! addslashes(old('lore_text', $achievement->lore_text)) !!}`,
        criteriaType: `{{ old('criteria_type', $achievement->criteria_type) }}`,
        criteriaValue: `{{ old('criteria_value', $achievement->criteria_value) }}`,
        xpReward: `{{ old('xp_reward', $achievement->xp_reward ?? 100) }}`,
        color: `{{ old('color', $achievement->color ?? '#eab308') }}`,
        icon: `{{ old('icon', $achievement->icon ?? 'fas fa-crown') }}`,
        isActive: {{ $achievement->is_active ? 'true' : 'false' }},
        previewSrc: `{{ $achievement->icon_url ?? '' }}`,
        colors: [
