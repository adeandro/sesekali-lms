...
@section('title', 'Edit Tema — Gamification Center')
@section('page-title', 'Edit Tema')
...
<div x-data="{ 
    ...
    isActive: {{ $theme->is_active ? 'true' : 'false' }}
}" class="max-w-6xl mx-auto pb-12 animate-fadeIn">

    <form action="{{ route('admin.gamification.themes.update', $theme) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        @csrf
        {{-- Custom override for update if needed --}}
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="flex items-center gap-4 mb-2">
                ...
                <div>
                    <h2 class="text-2xl font-black tracking-tight text-gray-900">Edit Tema: <span class="text-indigo-600">{{ $theme->name }}</span></h2>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Sesuaikan identitas visual dan variabel warna tema</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="number" name="min_level" value="{{ $theme->min_level }}" min="0" 
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="required_achievement_id" 
                            ...
                            @foreach($achievements as $achievement)
                                <option value="{{ $achievement->id }}" {{ $theme->required_achievement_id == $achievement->id ? 'selected' : '' }}>
                                    {{ $achievement->title }} ({{ $achievement->slug }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="p-5 rounded-2xl bg-gray-50 border border-gray-100">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <div class="relative">
                                <input type="checkbox" name="is_active" x-model="isActive" class="peer sr-only">
        ...
        <div class="lg:col-span-5">
            <div class="sticky top-12 space-y-6">
                ...
                <div class="bg-amber-50 border border-amber-100 p-6 rounded-[2rem] space-y-3">
                    <div class="flex items-center gap-2 text-amber-600">
                        <i class="fas fa-info-circle"></i>
                        <span class="text-[10px] font-black uppercase tracking-widest">Informasi Edit</span>
                    </div>
                    <p class="text-xs text-amber-900/70 leading-relaxed font-medium">
                        Perubahan warna pada tema yang sudah aktif akan langsung dirasakan oleh siswa yang menggunakan tema ini setelah mereka melakukan *refresh* halaman.
                    </p>
                </div>
            </div>
        </div>
