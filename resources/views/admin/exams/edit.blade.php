...
@section('title', 'Edit Ujian - ' . ($configs['school_name'] ?? 'SesekaliCBT'))
...
<div class="max-w-4xl mx-auto space-y-8 animate-fadeIn pb-12">
    ...
    <div class="flex flex-col gap-2">
        <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400">
            ...
            <span class="text-indigo-600">ID: #{{ $exam->id }}</span>
        </nav>
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-black text-gray-900 tracking-tight uppercase line-clamp-1">Edit: {{ $exam->title }}</h2>
    ...
    @if (!$exam->canEdit())
        <div class="bg-rose-50 border-2 border-rose-100 rounded-[2.5rem] p-8 flex items-start gap-6 shadow-sm">
            <div class="w-14 h-14 rounded-2xl bg-white flex items-center justify-center text-rose-600 shadow-md shrink-0">
                <i class="fas fa-exclamation-triangle text-xl"></i>
            </div>
            <div class="space-y-1">
                <h4 class="text-sm font-black text-rose-900 uppercase tracking-widest">Akses Terbatas</h4>
                <p class="text-[11px] font-bold text-rose-700/80 leading-relaxed uppercase">Ujian ini sudah selesai dilaksanakan atau sedang berlangsung. Beberapa parameter krusial telah dikunci untuk menjaga integritas data hasil ujian.</p>
            </div>
        </div>
    @endif

    <form action="{{ route('admin.exams.update', $exam) }}" method="POST" id="examForm" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="p-8 space-y-8">
                <div class="space-y-2">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $exam->title) }}" 
                        class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-gray-300 @error('title') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                        placeholder="Contoh: Penilaian Akhir Semester Ganjil" required {{ !$exam->canEdit() ? 'disabled' : '' }}>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <select id="subject_id" name="subject_id" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-xs font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer @error('subject_id') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <select id="jenjang" name="jenjang" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-xs font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer @error('jenjang') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                                required {{ !$exam->canEdit() ? 'disabled' : '' }}>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-500">
            ...
            <div class="p-8 space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="duration_minutes" name="duration_minutes" value="{{ old('duration_minutes', $exam->duration_minutes) }}" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('duration_minutes') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="total_questions" name="total_questions" value="{{ old('total_questions', $exam->total_questions) }}" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('total_questions') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                    ...
                    <div class="space-y-4">
                        <label for="status" class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Status Publikasi <span class="text-rose-500">*</span></label>
                        <div class="relative">
                            <select id="status" name="status" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-xs font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer @error('status') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                                required {{ !$exam->canEdit() ? 'disabled' : '' }}>
                                <option value="draft" {{ old('status', $exam->status) == 'draft' ? 'selected' : '' }}>DRAFT (Sembunyikan)</option>
                                <option value="published" {{ old('status', $exam->status) == 'published' ? 'selected' : '' }}>AKTIF (Tampilkan)</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-gray-400">
                                <i class="fas fa-chevron-down text-xs"></i>
                            </div>
                    ...
                    <div class="md:col-span-1 lg:col-span-1.5 space-y-4">
                        ...
                        <input type="datetime-local" id="start_time" name="start_time" 
                            ...
                            class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('start_time') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                            required {{ !$exam->canEdit() ? 'disabled' : '' }}>
                    ...
                    <div class="md:col-span-1 lg:col-span-1.5 space-y-4">
                        ...
                        <input type="datetime-local" id="end_time" name="end_time" 
                            ...
                            class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('end_time') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                            required {{ !$exam->canEdit() ? 'disabled' : '' }}>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-full grid grid-cols-1 md:grid-cols-2 gap-8 mb-4">
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="weight_pg" name="weight_pg" value="{{ old('weight_pg', $exam->weight_pg) }}" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('weight_pg') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                                min="0" max="100" required {{ !$exam->canEdit() ? 'disabled' : '' }}>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <input type="number" id="weight_essay" name="weight_essay" value="{{ old('weight_essay', $exam->weight_essay) }}" 
                                class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-sm font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all @error('weight_essay') ring-2 ring-rose-500 @enderror disabled:opacity-50" 
                                min="0" max="100" required {{ !$exam->canEdit() ? 'disabled' : '' }}>
                            <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-300 uppercase tracking-widest">%</span>
                        </div>
                ...
                <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all cursor-pointer overflow-hidden shadow-sm hover:shadow-lg hover:shadow-indigo-500/5 {{ !$exam->canEdit() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    <input type="checkbox" name="randomize_questions" value="1" {{ old('randomize_questions', $exam->randomize_questions) ? 'checked' : '' }} class="hidden peer" {{ !$exam->canEdit() ? 'disabled' : '' }}>
                ...
                <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all cursor-pointer overflow-hidden shadow-sm hover:shadow-lg hover:shadow-indigo-500/5 {{ !$exam->canEdit() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    <input type="checkbox" name="randomize_options" value="1" {{ old('randomize_options', $exam->randomize_options) ? 'checked' : '' }} class="hidden peer" {{ !$exam->canEdit() ? 'disabled' : '' }}>
                    <div class="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-gray-400 group-hover:text-indigo-600 transition-colors peer-checked:bg-indigo-600 peer-checked:text-white mr-5 shadow-sm">
                ...
                <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all cursor-pointer overflow-hidden shadow-sm hover:shadow-lg hover:shadow-indigo-500/5 {{ !$exam->canEdit() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    <input type="checkbox" name="show_score_after_submit" value="1" {{ old('show_score_after_submit', $exam->show_score_after_submit) ? 'checked' : '' }} class="hidden peer" {{ !$exam->canEdit() ? 'disabled' : '' }}>
                    <div class="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-gray-400 group-hover:text-indigo-600 transition-colors peer-checked:bg-indigo-600 peer-checked:text-white mr-5 shadow-sm">
                ...
                <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-indigo-100 hover:bg-white transition-all cursor-pointer overflow-hidden shadow-sm hover:shadow-lg hover:shadow-indigo-500/5">
                    <input type="checkbox" name="allow_review_results" value="1" {{ old('allow_review_results', $exam->allow_review_results) ? 'checked' : '' }} class="hidden peer">
                    <div class="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-gray-400 group-hover:text-indigo-600 transition-colors peer-checked:bg-indigo-600 peer-checked:text-white mr-5 shadow-sm">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-500"
            ...
            <div class="p-8 border-b border-gray-50 flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-base font-black text-gray-900 tracking-tight uppercase">Klasifikasi Raport</h3>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed">Tentukan tipe ujian dan hubungannya dengan penilaian raport semester</p>
                </div>
            </div>
            <div class="p-8 space-y-8">
                {{-- Field: Tipe Ujian --}}
                <div class="space-y-4">
                    <label for="exam_type" class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Tipe Ujian <span class="text-rose-500">*</span></label>
                    <div class="relative">
                        <select id="exam_type" name="exam_type"
                            x-model="examType"
                            class="w-full h-14 bg-gray-50 border-transparent rounded-2xl px-6 text-xs font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer @error('exam_type') ring-2 ring-rose-500 @enderror"
                            required>
                            @foreach(\App\Models\Exam::examTypes() as $value => $label)
                                <option value="{{ $value }}" {{ old('exam_type', $exam->exam_type ?? 'harian') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        <div class="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-gray-400">
                            <i class="fas fa-chevron-down text-xs"></i>
                    ...
                    @error('exam_type')<p class="text-rose-500 text-[10px] font-bold mt-2 ml-1 italic">{{ $message }}</p>@enderror
                </div>

                {{-- Conditional: Semester, Tahun Ajaran, Include in Report --}}
                <div x-show="examType !== 'latihan'" x-cloak class="space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {{-- Semester --}}
                        <div class="space-y-4">
                            ...
                            <div class="relative">
                                <select id="semester" name="semester"
                                    ...
                                    <option value="1" {{ old('semester', $exam->semester) == 1 ? 'selected' : '' }}>Semester 1 (Ganjil)</option>
                                    <option value="2" {{ old('semester', $exam->semester) == 2 ? 'selected' : '' }}>Semester 2 (Genap)</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-gray-400">
                                    <i class="fas fa-chevron-down text-xs"></i>
                                </div>
                        ...
                        {{-- Tahun Ajaran --}}
                        <div class="space-y-4">
                            <label for="academic_year_report" class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Tahun Ajaran <span class="text-rose-500">*</span></label>
                            <input type="text" id="academic_year_report" name="academic_year"
                                value="{{ old('academic_year', $exam->academic_year ?? $configs['academic_year'] ?? '') }}"
                                :required="examType !== 'latihan'"
                    ...
                    <label class="group relative flex items-center p-6 bg-gray-50 rounded-[2rem] border-2 border-transparent hover:border-emerald-400 hover:bg-white transition-all duration-200 cursor-pointer overflow-hidden shadow-sm hover:shadow-lg">
                        <input type="checkbox" name="include_in_report" value="1" {{ old('include_in_report', $exam->include_in_report ?? true) ? 'checked' : '' }} class="hidden peer">
        ...
        <div class="flex flex-col-reverse md:flex-row gap-4 pt-4">
            ...
            @if($exam->canEdit())
                <button type="submit" class="flex-[2] h-14 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 flex items-center justify-center gap-3 group">
                    <i class="fas fa-save text-[10px] group-hover:scale-110 transition-transform"></i> Perbarui Data Ujian
                </button>
            @endif
        </div>
    </form>
...
<div id="loadingOverlay" class="fixed inset-0 bg-white/80 backdrop-blur-sm z-[100] hidden flex items-center justify-center animate-fadeIn">
    <div class="text-center space-y-6">
        ...
        <div class="space-y-1">
            ...
            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Menyimpan perubahan ujian...</p>
        </div>
    </div>
</div>
