...
class User extends Authenticatable
{
    ...
    {
        // 1. If student uploaded their own photo (via student profile)
        if ($this->avatar_upload) {
            return '/storage/' . str_replace(' ', '%20', $this->avatar_upload);
        }

        // 2. Check for uploaded photo from Admin (profiles/ folder)
        if ($this->photo && \Illuminate\Support\Facades\Storage::disk('public')->exists('profiles/' . $this->photo)) {
            return '/storage/profiles/' . str_replace(' ', '%20', $this->photo);
        }

        // 3. If student chose an avatar from gallery or generated by Multiavatar (Legacy)
        ...
        if ($globalDefault && \Illuminate\Support\Facades\Storage::disk('public')->exists($globalDefault)) {
            return '/storage/' . str_replace(' ', '%20', $globalDefault);
        }
